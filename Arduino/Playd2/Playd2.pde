...
void loop() {
  ...
  if (modeButtonVal==HIGH){
        ...
        if (currentMode==0){
          ...
          currentMode=3;
          modeSwitch(currentMode);
          Serial.println("modechange BRRRRR");
          vibrateLong(); 
          mode2Mode=0;
        }
 ...
 if (valB2 >= THRESHOLD) {
  ...
  if (currentMode==3){
    ...
    // push B2 mode
      ...
      if (millis() - curTime <flashinterval){
            digitalWrite(ledPinB2RY,HIGH);             
            digitalWrite(ledPinB2G,LOW); 
       }
      else if (millis() - curTime < (flashinterval*2)){
...
void vibrateLong(){
  long viblength = 4000;
  long flashinterval1 = 100;
  long curTime = millis();
  long endTime = curTime+viblength;
  
  digitalWrite(vibraPin,HIGH);
      
  while( millis() < endTime){
    if (millis() - curTime <flashinterval1){
            digitalWrite(ledPinC3Y,HIGH);             
            digitalWrite(ledPinB2RY,HIGH); 
            digitalWrite(ledPinC3R,LOW);  
            digitalWrite(ledPinB2G,LOW);
    }
    else if (millis() - curTime < (flashinterval1*2)){
            digitalWrite(ledPinC3Y,LOW);          
            digitalWrite(ledPinB2RY,LOW); 
            digitalWrite(ledPinC3R,HIGH);  
            digitalWrite(ledPinB2G,HIGH);
  ...
  Serial.println("BRR uit");
}

// modeswitch

void modeSwitch(int cm){
          resetLights();
          Serial.print("Mode switched to ");
          Serial.println(currentMode);
          if(cm==0){
            playNote(speakerPinL, 'c', 200);
            playNote(speakerPinR, 'c', 200);
